...
export default function EditTable({ onClose , onUpdateSuccess, id}) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/table/update/${id}`, {
        method: 'PUT',
        headers: {
      ...
      if (response.status  === 200) {
        ...
        toast.error(data.message || 'Sửa bàn thất bại');
      }
    } catch (error) {
