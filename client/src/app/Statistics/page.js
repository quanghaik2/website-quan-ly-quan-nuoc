...
const [endDate, setEndDate] = useState(end.toISOString().split('T')[0]); // Định dạng YYYY-MM-DD
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response2 = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/statistics/monthly`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  return (
    <div className="container mx-auto p-6 bg-white text-black">
      {type === 'month' ? (
        <div><h1 className="text-2xl font-bold mb-4">Thống Kê Theo Tháng</h1>
      ...
      <div className='text-2xl font-bold mt-9 mb-9'>Chi tiết số sản phẩm đã bán: </div>
      <table className="min-w-full bg-white border border-gray-200 rounded-md shadow-md">
        <thead>
          <tr>
            <th className="py-2 px-4 border-b">STT</th>
      ...
      <table className="min-w-full bg-white border border-gray-200 rounded-md shadow-md">
        <thead>
          <tr>
            <th className="py-2 px-4 border-b">STT</th>
            <th className="py-2 px-4 border-b">Thời gian lên đơn</th>
            <th className="py-2 px-4 border-b">Tổng tiền của đơn</th>
            <th className="py-2 px-4 border-b">Trạng thái của đơn</th>
          </tr>
        </thead>
        {orders.map((order, index) => (
          <tbody>
            <tr key={index}>
              ...
              <th className="py-2 px-4 font-mono">{order.total_amount} </th>
              <th ><button className={`w-36 py-1 rounded-lg font-mono text-white ${order.status === 'đã lên đơn' ? 'bg-blue-300' : order.status === 'chờ thanh toán' ? 'bg-red-300' : 'bg-green-300'}`}>{order.status}</button> </th>
            </tr>
          </tbody>
        ))}
        <tbody>
